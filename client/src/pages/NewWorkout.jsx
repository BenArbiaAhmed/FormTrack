...
export default function NewWorkout() {
  ...
  useEffect(() => {
    if (workoutStatus === "running" && sessionId) {
      eventSourceRef.current = new EventSource(
        `http://localhost:8000/rep-count/${sessionId}`
      )
  ...
  const handleStart = async () => {
    ...
    try {
      await axios.post(`/start-session/${newSessionId}/${currentExercise}`);      
  ...
  const handleStop = async () => {
    ...
    try {
      const response = await axios.post(`/end-session/${sessionId}`); 
      setSessionId(null)
  ...
  const handleExerciseChange = async (value) => {
    ...
    try {
      await axios.post(`/start-session/${newSessionId}/${value}`);
      setSessionId(newSessionId)
      setExerciseStartTime(new Date())
      setWorkoutStatus("running")
    } catch (error) {
          ...
          <div className="@container/main flex flex-1 flex-col gap-2">
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6 items-center">
              {workoutStatus === "running" && sessionId && (
                <img 
                  src={`http://localhost:8000/video/${sessionId}/${currentExercise}`} 
                  alt="Video feed"
